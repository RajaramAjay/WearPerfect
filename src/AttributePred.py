...
# Model and encoder filenames
bottom_wear_model_files = [
    "best_bottomwear_model.keras"
]

bottom_wear_encoder_files = [
    "bottom_length_encoder.pkl"
]
...
def get_all_attribute_predictions(image_path, clothing_type):
    ...
    if clothing_type.lower() == 'top':
        ...
        for i, (model, encoder, attr_name) in enumerate(zip(top_wear_models, top_wear_encoders, top_wear_attribute_names)):
            ...
            try:
                # Make prediction
                pred_probs = model.predict(img_batch, verbose=0)  # Set verbose=0 to suppress progress bar
        ...
        for i, (model, encoder, attr_name) in enumerate(zip(bottom_wear_models, bottom_wear_encoders, bottom_wear_attribute_names)):
            if model is None or encoder is None:
