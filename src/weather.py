...
def get_datecity_forecast(city, date, event=None):
    ...
    try:
        ...
        w_data = {
            ...
            "Atmospheric Pressure": data["current"]["pressure_mb"] if "current" in data else "N/A",
            "UV Index": uv_index,
            "Visibility (km)": data["current"]["vis_km"] if "current" in data else "N/A",
            "Location": [f"{data['location']['name']}, {data['location']['country']}"],
            "Season": [season_map[month]]
        }

        prediction = predict_weather(w_data)
        top_wear_items=read_clothing_csv(top_wear_csv,prediction)
        bottom_wear_items=read_clothing_csv(bottom_wear_csv,prediction)
